...
double easeInSine(double t) { return std::sin((PI / 2.0) * t); }

double easeOutSine(double t) {
    t--;
    return 1 + std::sin((PI / 2.0) * t);
}

double easeInOutSine(double t) { return 0.5 * (1 + std::sin(PI * (t - 0.5))); }
...
double easeInOutCubic(double t) { return t < 0.5 ? 4 * t * t * t : 1 + (t - 1) * (2 * (t - 2)) * (2 * t - 2); }
...
double easeInExpo(double t) { return (std::pow(2, 8 * t) - 1) / 255; }

double easeOutExpo(double t) { return 1 - std::pow(2, -8 * t); }
...
double easeInCirc(double t) { return 1 - std::sqrt(1 - t); }

double easeOutCirc(double t) { return std::sqrt(t); }
...
double easeInBounce(double t) { return std::pow(2, 6 * (t - 1)) * std::abs(std::sin(t * PI * 3.5)); }

double easeOutBounce(double t) { return 1 - std::pow(2, -6 * t) * std::abs(std::cos(t * PI * 3.5)); }
